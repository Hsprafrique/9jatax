...
export default function Staff() {
  ...
  async function saveStaff(e) {
    ...
    if (error) { console.error('Staff insert error:', error); setSaving(false); return }
    ...
    <div className="p-4 lg:p-6 max-w-5xl space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="stat-card rounded-2xl p-4 border border-white/10">
          <p className="text-xs text-white/40 mb-1">Payroll records</p>
      ...
      <div className="flex items-center justify-between">
        <div className="flex gap-1 glass rounded-xl p-1">
          <button onClick={() => setTab('staff')} className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${tab === 'staff' ? 'bg-[#008751] text-white' : 'text-white/40 hover:text-white'}`}>Staff</button>
          <button onClick={() => setTab('payroll')} className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${tab === 'payroll' ? 'bg-[#008751] text-white' : 'text-white/40 hover:text-white'}`}>Payroll history</button>
        </div>
        {tab === 'staff' && (
        ...
        <div className="navy-card">
          {loading ? (
            ...
            <div className="py-14 text-center">
              <UserCheck size={32} className="text-white/20 mx-auto mb-2" />
              <p className="text-sm text-white/30">No staff members yet</p>
            </div>
          ) : (
            <div className="divide-y divide-white/5">
              {staff.map(s => (
                <div key={s.id} className="flex items-center px-5 py-4 hover:bg-white/2 transition-colors">
                  <div className="w-10 h-10 rounded-full bg-[#008751]/15 border border-emerald-500/20 flex items-center justify-center shrink-0 mr-3">
                    <span className="text-green-bright font-bold text-sm">{s.name[0].toUpperCase()}</span>
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-white text-sm">{s.name}</p>
                    <div className="flex items-center gap-3 mt-0.5">
                      <span className="text-xs text-white/30 capitalize">{s.role}</span>
                      {s.phone && <span className="text-xs text-white/30 flex items-center gap-1"><Phone size={10} />{s.phone}</span>}
                    </div>
                  </div>
                  <div className="text-right mr-4">
                    <p className="font-mono font-semibold text-white text-sm">{formatCurrency(s.salary)}</p>
                    <p className="text-xs text-white/30 capitalize">/{s.salary_type}</p>
                  </div>
                  <div className="flex items-center gap-2">
                    <button onClick={() => setShowPayroll(s)}
                      className="text-xs bg-[#008751]/15 text-green-bright hover:bg-[#008751]/25 px-2.5 py-1.5 rounded-lg transition-colors">
                      Pay
                    </button>
                    <button onClick={() => remove(s.id)} className="text-white/20 hover:text-red-400 transition-colors"><Trash2 size={14} /></button>
                  </div>
                </div>
              ))}
            </div>
      ...
      {tab === 'payroll' && (
        <div className="navy-card">
          {payroll.length === 0 ? (
            <div className="py-14 text-center">
              <DollarSign size={32} className="text-white/20 mx-auto mb-2" />
              <p className="text-sm text-white/30">No payroll records yet</p>
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-white/5 bg-white/4/50">
                    <th className="text-left px-5 py-3 text-xs font-medium text-white/40">Staff</th>
                    <th className="text-left px-5 py-3 text-xs font-medium text-white/40 hidden sm:table-cell">Period</th>
                    <th className="text-right px-5 py-3 text-xs font-medium text-white/40">Base</th>
                    <th className="text-right px-5 py-3 text-xs font-medium text-white/40 hidden md:table-cell">Bonus</th>
                    <th className="text-right px-5 py-3 text-xs font-medium text-white/40 hidden md:table-cell">Deduct</th>
                    <th className="text-right px-5 py-3 text-xs font-medium text-white/40">Net pay</th>
                ...
                <tbody className="divide-y divide-white/5">
                  {payroll.map(p => (
                    <tr key={p.id} className="table-row-hover transition-colors">
                      ...
                      <td className="px-5 py-3 text-white/40 hidden sm:table-cell text-xs">{formatDate(p.period_start)} --- {formatDate(p.period_end)}</td>
                      <td className="px-5 py-3 text-right font-mono text-white/60">{formatCurrency(p.base_salary)}</td>
                      <td className="px-5 py-3 text-right font-mono text-green-bright hidden md:table-cell">+{formatCurrency(p.bonuses)}</td>
                      ...
                      <td className="px-5 py-3">
                        <span className={`text-xs px-2 py-1 rounded-full ${p.status === 'paid' ? 'bg-[#008751]/15 text-green-bright' : 'bg-amber-500/15 text-amber-400'}`}>{p.status}</span>
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            <form onSubmit={saveStaff} className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select value={form.role} onChange={e => setForm(f => ({...f, role: e.target.value}))} className="input-dark bg-black/30">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select value={form.salary_type} onChange={e => setForm(f => ({...f, salary_type: e.target.value}))} className="input-dark bg-black/30">
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="modal-panel rounded-2xl w-full max-w-sm shadow-2xl">
            <div className="flex items-center justify-between px-6 py-4 border-b" style={{borderColor:"rgba(255,255,255,0.08)"}}>
              ...
              <button onClick={() => setShowPayroll(null)} className="text-white/40 hover:text-white"><X size={18} /></button>
            </div>
            <form onSubmit={processPayroll} className="p-6 space-y-4">
              <div className="glass rounded-xl p-3 text-sm space-y-1">
                <p className="text-white/40">Staff: <span className="text-white font-medium">{showPayroll.name}</span></p>
                <p className="text-white/40">Base salary: <span className="text-green-bright font-bold">{formatCurrency(showPayroll.salary)}/{showPayroll.salary_type}</span></p>
                <p className="text-white/40">Period: <span className="text-white">{format(startOfMonth(new Date()), 'MMM d')} --- {format(endOfMonth(new Date()), 'MMM d, yyyy')}</span></p>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
              ...
              <div className="glass rounded-xl p-3 flex justify-between items-center">
                <span className="text-white/40 text-sm">Net pay</span>
                <span className="text-green-bright font-bold text-lg font-mono">
