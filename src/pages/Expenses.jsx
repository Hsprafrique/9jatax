...
export default function Expenses() {
  ...
  async function save(e) {
    ...
    if (editing) {
      const { error } = await supabase.from('expenses').update(payload).eq('id', editing.id).eq('user_id', user.id)
      if (error) { console.error('Expense update error:', error); setSaving(false); return }
    } else {
      const { error } = await supabase.from('expenses').insert({ ...payload, user_id: user.id })
      if (error) { console.error('Expense insert error:', error); setSaving(false); return }
    }
  ...
  return (
    <div className="p-4 lg:p-6 max-w-5xl space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-white/40">{expenses.length} entries -- <span className="text-white font-semibold">{formatCurrency(total)}</span></p>
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        ...
        <select value={catFilter} onChange={e => setCatFilter(e.target.value)}
          className="bg-white/5 border border-white/12 text-white/75 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500">
      ...
      <div className="navy-card white-top-border rounded-2xl overflow-hidden">
        {loading ? (
          ...
          <div className="py-16 text-center">
            <Receipt size={32} className="text-white/20 mx-auto mb-2" />
            <p className="text-sm text-white/30">No expenses found</p>
            <button onClick={openNew} className="mt-3 text-sm text-green-bright hover:text-green-bright">Add your first expense ---</button>
          </div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-white/5 bg-white/4/50">
                  ...
                  <th className="text-left px-4 py-3 text-xs font-medium text-white/40 hidden sm:table-cell">Category</th>
                  <th className="text-left px-4 py-3 text-xs font-medium text-white/40 hidden md:table-cell">Vendor</th>
                  <th className="text-right px-4 py-3 text-xs font-medium text-white/40">Amount</th>
              ...
              <tbody className="divide-y divide-white/5">
                {filtered.map(exp => (
                  <tr key={exp.id} onClick={() => openEdit(exp)} className="hover:bg-white/2 cursor-pointer transition-colors">
                    <td className="px-4 py-3 text-white/40 whitespace-nowrap">{formatDate(exp.date)}</td>
                    <td className="px-4 py-3 font-medium text-white">{exp.description}</td>
                    <td className="px-4 py-3 text-white/40 hidden sm:table-cell">{CATEGORIES.find(c => c.value === exp.category)?.label || exp.category}</td>
                    <td className="px-4 py-3 text-white/40 hidden md:table-cell">{exp.vendor || '---'}</td>
                    <td className="px-4 py-3 text-right font-mono font-semibold text-white">{formatCurrency(exp.amount)}</td>
                    <td className="px-4 py-3">
                      <button onClick={e => { e.stopPropagation(); remove(exp.id) }} className="text-white/20 hover:text-red-400 transition-colors"><Trash2 size={14} /></button>
                    </td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr className="bg-white/4/50 border-t border-white/10">
                  <td colSpan={4} className="px-4 py-3 text-sm font-medium text-white/60">Total ({filtered.length} items)</td>
          ...
          <div className="modal-panel rounded-2xl w-full max-w-md shadow-2xl">
            ...
            <form onSubmit={save} className="p-6 space-y-4">
              ...
              <div>
                ...
                <select value={form.category} onChange={e => setForm(f => ({...f, category: e.target.value}))} className="input-dark bg-black/30">
                  {CATEGORIES.map(c => <option key={c.value} value={c.value}>{c.label}</option>)}
                </select>
              </div>
              <div>
