  ...
  return (
    <div className="p-4 lg:p-6 max-w-4xl space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-white/40">{branches.length} branch{branches.length !== 1 ? 'es' : ''}</p>
      ...
      {/* Info banner */}
      <div className="glass rounded-xl px-4 py-3 border border-emerald-500/20 text-sm text-white/60">
        <span className="text-green-bright font-medium">Multi-branch:</span> Add your shop locations here. When creating invoices or recording expenses, you can tag them to a specific branch for separate reporting.
      </div>

      <div className="grid md:grid-cols-2 gap-4">
        {loading ? (
          [1,2].map(i => <div key={i} className="navy-card white-top-border h-40 animate-pulse rounded-2xl" />)
        ) : branches.length === 0 ? (
          <div className="md:col-span-2 navy-card py-16 text-center rounded-2xl">
            <GitBranch size={32} className="text-white/20 mx-auto mb-2" />
            <p className="text-sm text-white/30">No branches yet</p>
            <button onClick={() => setShowForm(true)} className="mt-3 text-sm text-green-bright hover:text-green-bright">Add your first branch ---</button>
          </div>
        ) : (
          branches.map(b => (
              ...
              <div className="flex items-start justify-between mb-3">
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    {b.is_default && (
                      <span className="text-xs bg-[#008751]/15 text-green-bright px-2 py-0.5 rounded-full border border-emerald-500/20 flex items-center gap-1">
                        <Star size={10} fill="currentColor" /> Default
                      </span>
                    )}
                  </div>
                  {b.address && <p className="text-xs text-white/40 mt-1 flex items-center gap-1"><MapPin size={10} />{b.address}</p>}
                  {b.phone && <p className="text-xs text-white/40 mt-0.5 flex items-center gap-1"><Phone size={10} />{b.phone}</p>}
                </div>
                <button onClick={() => remove(b.id)} className="text-white/20 hover:text-red-400 transition-colors">
                  <Trash2 size={14} />
                </button>
              </div>
              {!b.is_default && (
                <button onClick={() => setDefault(b.id)} className="text-xs text-white/40 hover:text-green-bright transition-colors flex items-center gap-1 mt-2">
            ...
            <form onSubmit={save} className="p-6 space-y-4">
              ...
              <div className="flex items-center justify-between p-3 glass rounded-xl border border-white/5">
                <div>
                  ...
                  <p className="text-xs text-white/40">New transactions tagged here by default</p>
                </div>
                <button type="button" onClick={() => setForm(f => ({...f, is_default: !f.is_default}))}
                  className={`w-10 h-6 rounded-full transition-colors ${form.is_default ? 'bg-[#008751]' : 'bg-navy-600'}`}>
