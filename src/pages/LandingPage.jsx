...
import {
  ...
  BarChart2, UserCheck, CreditCard, Bell, Smartphone, Globe, ArrowLeftRight
} from 'lucide-react'
...
const FEATURES = [
  { icon: TrendingUp, title: 'Income & Sales Tracking', desc: 'Create invoices, record payments and know your revenue in real time.', color: 'text-green-bright', bg: 'bg-emerald-400/10' },
  ...
  { icon: ArrowLeftRight, title: 'Transaction Ledger', desc: 'Unified view of every naira in and out --- income, expenses, and transfers in one place.', color: 'text-cyan-400', bg: 'bg-cyan-400/10' },
  { icon: Shield, title: 'Tax Compliance (FIRS)', desc: 'Track VAT, PAYE, WHT and CIT. Stay FIRS-ready with filing reminders and a VAT calculator.', color: 'text-green-400', bg: 'bg-green-400/10' },
  { icon: BarChart2, title: 'Financial Reports', desc: 'P&L statements, profit trends, expense breakdowns and top customer reports. Export to CSV.', color: 'text-indigo-400', bg: 'bg-indigo-400/10' },
  ...
  { icon: Printer, title: 'Print Anywhere', desc: 'Print professional invoices and expense reports directly from your browser.', color: 'text-green-bright', bg: 'bg-emerald-400/10' },
...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-navy-950 font-sans overflow-x-hidden">
      ...
      {/* NAV */}
      <nav className="fixed top-0 left-0 right-0 z-50 glass border-b" style={{borderColor:"rgba(255,255,255,0.08)"}}>
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-3">
            <Link to="/auth" className="text-sm font-medium text-white/60 hover:text-white px-4 py-2 transition-colors">Log in</Link>
        ...
        {menuOpen && (
          <div className="md:hidden glass border-t border-white/5 px-4 py-4 space-y-2">
            {['Features','How it works','About'].map(l => (
              <a key={l} href={`#${l.toLowerCase().replace(/ /g,'-')}`} onClick={() => setMenuOpen(false)} className="block text-sm text-white/60 py-2 hover:text-white">{l}</a>
            ))}
            <div className="flex gap-3 pt-3 border-t border-white/10">
              ...
              <Link to="/auth?mode=signup" onClick={() => setMenuOpen(false)} className="flex-1 text-center text-sm font-semibold bg-[#008751] text-white py-2.5 rounded-xl">Sign up free</Link>
            </div>
          </div>
        )}
      </nav>
      ...
      <section className="relative min-h-screen flex items-center pt-16 overflow-hidden">
        ...
        <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-[#008751]/5 rounded-full blur-3xl animate-pulse-slow" />
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-white/4/30 rounded-full blur-3xl" />

        <div className="relative max-w-6xl mx-auto px-4 py-20 lg:py-32 grid lg:grid-cols-2 gap-16 items-center w-full">
          <div className="animate-fade-up">
            <div className="inline-flex items-center gap-2 glass rounded-full px-4 py-2 text-sm font-medium text-green-bright mb-6 glow-green-sm">
            ...
            <div className="flex flex-wrap items-center gap-5 text-sm text-white/40">
              {['Free to start', 'No credit card', 'Works on mobile'].map(t => (
                <span key={t} className="flex items-center gap-1.5">
                  <CheckCircle size={14} className="text-green-bright" /> {t}
                </span>
              ))}
            </div>
          </div>
          ...
          <div className="hidden lg:block animate-fade-up" style={{animationDelay:'0.2s'}}>
            <div className="glass-card rounded-3xl p-6 glow-green-sm animate-float">
              <div className="flex items-center justify-between mb-5">
                <div>
                  <p className="text-white/40 text-xs mb-0.5">Welcome back</p>
              ...
              <div className="grid grid-cols-2 gap-3 mb-4">
                {[
                  { label: 'Total Income', value: '---248,000', color: 'text-green-bright', icon: '----' },
                  { label: 'Expenses', value: '---91,500', color: 'text-red-400', icon: '----' },
                  { label: 'Net Profit', value: '---156,500', color: 'text-green-bright', icon: '----' },
              ...
              <div className="glass rounded-xl p-3 mb-3">
                <p className="text-white/40 text-xs mb-2">Recent sales</p>
                {[
                  ...
                  <div key={inv.name} className="flex items-center justify-between py-1.5 border-b border-white/5 last:border-0">
                    <span className="text-sm text-white/75">{inv.name}</span>
                    <div className="flex items-center gap-2">
                      <span className={`text-xs px-1.5 py-0.5 rounded-full ${inv.status === 'paid' ? 'bg-[#008751]/20 text-green-bright' : 'bg-amber-500/20 text-amber-400'}`}>{inv.status}</span>
              ...
              <div className="flex items-center gap-2 glass rounded-xl px-3 py-2">
                ...
                <p className="text-xs text-white/60">2 low stock alerts -- 1 debt due today</p>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* ANIMATED STATS */}
      <section ref={statsRef} className="py-16 border-y" style={{background:"linear-gradient(135deg,rgba(255,255,255,0.05) 0%,rgba(10,35,20,0.95) 100%)",borderColor:"rgba(255,255,255,0.08)"}}>
      ...
      <section id="features" className="py-24 bg-navy-950">
        <div className="max-w-6xl mx-auto px-4">
          <div className="text-center mb-16">
            <div className="inline-flex items-center gap-2 glass rounded-full px-4 py-1.5 text-sm text-green-bright mb-4">
            ...
            <p className="text-white/40 text-lg max-w-xl mx-auto">Built around how Nigerian businesses actually work --- not how accountants think you should.</p>
          </div>
      ...
      {/* HOW IT WORKS */}
      <section id="how-it-works" className="py-24 bg-black/30">
        <div className="max-w-6xl mx-auto px-4">
          <div className="text-center mb-16">
            <h2 className="text-3xl lg:text-4xl font-bold text-white mb-4">Up and running in minutes</h2>
            <p className="text-white/40 text-lg">If you can use WhatsApp, you can use 9jaTax.</p>
          </div>
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8 relative">
            ...
            {HOW.map((s, i) => (
              <div key={s.step} className="relative text-center">
                <div className="w-16 h-16 bg-[#008751]/10 border border-emerald-500/30 text-green-bright rounded-2xl flex items-center justify-center font-bold text-lg mx-auto mb-5 glow-green-sm">
                  {s.step}
                </div>
      ...
      <section className="py-24 bg-navy-950">
        <div className="max-w-6xl mx-auto px-4">
          <div className="text-center mb-16">
            ...
            <p className="text-white/40">Real results from real Nigerian businesses</p>
          </div>
          <div className="grid md:grid-cols-3 gap-6">
            {TESTIMONIALS.map(t => (
              <div key={t.name} className="white-top-border rounded-2xl p-6 transition-all" style={{background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.12)"}}>
                ...
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 bg-[#008751]/20 rounded-full flex items-center justify-center">
                    <span className="text-green-bright font-bold text-sm">{t.name[0]}</span>
                  </div>
                  <div>
      ...
      <section id="about" className="py-24 bg-black/30">
        <div className="max-w-4xl mx-auto px-4">
          <div className="text-center mb-14">
            ...
            <p className="text-white/40">The person behind 9jaTax</p>
          </div>
          <div className="glass-card rounded-3xl p-8 lg:p-10 flex flex-col md:flex-row items-center gap-10">
            ...
            <div>
              <div className="inline-flex items-center gap-2 bg-[#008751]/10 text-green-bright rounded-full px-4 py-1.5 text-sm font-medium mb-4 border border-emerald-500/20">
              ...
              <div className="mt-5 flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-white/40">Product & Technology</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <section className="py-24 bg-navy-950 relative overflow-hidden">
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-[#008751]/5 rounded-full blur-3xl" />
        <div className="relative max-w-3xl mx-auto px-4 text-center">
          ...
          <p className="text-white/30 text-sm mt-4">Takes less than 2 minutes</p>
        </div>
      </section>

      {/* FOOTER */}
      <footer style={{ background: '#111111', borderTop: '1px solid rgba(255,255,255,0.06)' }} className="py-10">
        <div className="ng-flag-stripe mb-8" />
        <div className="max-w-6xl mx-auto px-4">
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: 'linear-gradient(135deg,#008751,#006B40)' }}>
                <span className="text-white font-bold text-sm">9T</span>
              </div>
              <div>
                <p className="font-bold text-white text-sm">9jaTax</p>
                <p className="text-xs" style={{ color: 'rgba(255,255,255,0.3)' }}>by HSPR Technologies Ltd</p>
              </div>
            </div>
            <p className="text-sm text-center" style={{ color: 'rgba(255,255,255,0.3)' }}>
              &copy; {year} HSPR Technologies Limited (RC 7163476) - All rights reserved - Made in Nigeria
            </p>
            <div className="flex gap-5 text-sm" style={{ color: 'rgba(255,255,255,0.4)' }}>
