...
export default function Debts() {
  ...
  async function save(e) {
    ...
    if (error) { console.error('Debt insert error:', error); setSaving(false); return }
  ...
  const statusColor = { active: 'bg-red-500/15 text-red-400', partial: 'bg-amber-500/15 text-amber-400', settled: 'bg-[#008751]/15 text-green-bright' }
    ...
    <div className="p-4 lg:p-6 max-w-5xl space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="stat-card rounded-2xl p-4 border border-red-500/20">
          <p className="text-xs text-white/40 mb-1">{tab === 'owed_to_me' ? 'People owe you' : 'You owe'}</p>
        ...
        <div className="stat-card rounded-2xl p-4 border border-emerald-500/20">
          <p className="text-xs text-white/40 mb-1">Settled</p>
          <p className="text-xl font-bold text-green-bright">{totalSettled}</p>
        </div>
        <div className="stat-card rounded-2xl p-4 border border-white/10">
          <p className="text-xs text-white/40 mb-1">Total records</p>
      ...
      <div className="flex items-center justify-between">
        <div className="flex gap-1 glass rounded-xl p-1">
          <button onClick={() => setTab('owed_to_me')} className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${tab === 'owed_to_me' ? 'bg-[#008751] text-white' : 'text-white/40 hover:text-white'}`}>
            Owed to me
          </button>
          <button onClick={() => setTab('i_owe')} className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${tab === 'i_owe' ? 'bg-red-500 text-white' : 'text-white/40 hover:text-white'}`}>
            I owe
          </button>
        </div>
      ...
      <div className="navy-card">
        {loading ? (
          ...
          <div className="py-14 text-center">
            ...
            <p className="text-sm text-white/30">No {tab === 'owed_to_me' ? 'debts owed to you' : 'debts you owe'}</p>
          </div>
        ) : (
              ...
              return (
                <div key={d.id} className="flex items-center px-5 py-4 hover:bg-white/2 transition-colors">
                  <div className="flex-1 min-w-0">
                    ...
                    {d.due_date && <p className="text-xs text-white/30 mt-0.5">Due: {formatDate(d.due_date)}</p>}
                  </div>
                  <div className="text-right mr-4">
                    <p className="font-mono font-bold text-white text-sm">{formatCurrency(remaining)}</p>
                    {d.amount_paid > 0 && <p className="text-xs text-white/30">of {formatCurrency(d.amount)}</p>}
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => remove(d.id)} className="text-white/20 hover:text-red-400 transition-colors"><Trash2 size={14} /></button>
                  </div>
                </div>
              )
            })}
            ...
            <form onSubmit={save} className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-2">
                <button type="button" onClick={() => setForm(f => ({...f, type: 'owed_to_me'}))}
                  className={`py-2.5 rounded-xl text-sm font-medium border transition-all ${form.type === 'owed_to_me' ? 'bg-[#008751]/20 border-emerald-500/40 text-green-bright' : 'border-white/10 text-white/40 hover:border-white/20'}`}>
                  Owed to me
                </button>
                <button type="button" onClick={() => setForm(f => ({...f, type: 'i_owe'}))}
                  className={`py-2.5 rounded-xl text-sm font-medium border transition-all ${form.type === 'i_owe' ? 'bg-red-500/20 border-red-500/40 text-red-400' : 'border-white/10 text-white/40 hover:border-white/20'}`}>
                  I owe
                </button>
              </div>
              <div>
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="modal-panel rounded-2xl w-full max-w-sm shadow-2xl">
            <div className="flex items-center justify-between px-6 py-4 border-b" style={{borderColor:"rgba(255,255,255,0.08)"}}>
              ...
              <button onClick={() => setShowPayment(null)} className="text-white/40 hover:text-white"><X size={18} /></button>
            </div>
            <form onSubmit={recordPayment} className="p-6 space-y-4">
              <div className="glass rounded-xl p-3 text-sm">
                <p className="text-white/40">Party: <span className="text-white font-medium">{showPayment.party_name}</span></p>
                <p className="text-white/40 mt-1">Remaining: <span className="text-green-bright font-bold">{formatCurrency(showPayment.amount - (showPayment.amount_paid || 0))}</span></p>
              </div>
              <div>
